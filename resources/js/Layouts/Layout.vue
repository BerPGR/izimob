        ...
        <ul class="menu w-full grow gap-3">
          ...
          <li
            v-if="
              ...
              page.props.auth.user.role === 'master'
            "
          >
            <Link
              as="button"
...
const home = computed(() => {
  const superRoles = ["admin", "user", "master"];
  if (superRoles.includes(page.props.auth.user.role)) return "admin.dashboard";
  return "client.home";
});
</script>
